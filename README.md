...
- **Enhanced Detection**: ARM64 backward-slice parameter extraction fallback, caller-name heuristic checks, ObjC selector matching via `objc_msgSend`, string cross-referencing with security-critical call sites, and interprocedural call-chain analysis (BFS up to 3 hops). [Details](Trellis_Enhanced_Detection.md)
