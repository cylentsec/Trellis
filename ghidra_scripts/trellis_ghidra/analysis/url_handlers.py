...
(function() {{
    ...
    function getGhidraOffset(runtimeAddr, module) {{
        var runtimeOffset = runtimeAddr.sub(module.base);
        ...
        if (magic === 0xfeedfacf) {{  // MH_MAGIC_64
            ...
            return runtimeOffset.add(textVmaddr);
        }}
    ...
    function isSwiftSymbol(name) {{
        return name && (name.startsWith('$s') || name.startsWith('_$s'));
    }}

"""

    # Group handlers by type for organized output
    for handler in handlers:
        # Calculate offset for the hook
        is_swift = handler.symbol_name.startswith(
            "$s"
        ) or handler.symbol_name.startswith("_$s")

        # Extract a short identifier from the symbol for matching
        match_fragment = ""
        if "$s" in handler.symbol_name:
            parts = handler.symbol_name.split("$s")
            if len(parts) > 1:
                match_fragment = parts[-1][:20]
        else:
            match_fragment = handler.symbol_name[:20]

        script += f"""
    // Handler: {handler.demangled_name}
    // Type: {handler.handler_type} | Confidence: {handler.confidence}
    (function() {{
        var symbolName = "{handler.symbol_name}";
        var handlerType = "{handler.handler_type}";
        var isSwift = {'true' if is_swift else 'false'};

        // Find the function
        var funcPtr = null;

        // Try to find by export name first (Frida 17+)
        try {{
            var mainMod = Process.getModuleByName(mainModule.name);
            if (mainMod) {{
                funcPtr = mainMod.getExportByName(symbolName);
            }}
        }} catch (e) {{
            // Ignore and fall back to export enumeration
        }}

        if (!funcPtr) {{
            // Try to find by symbol enumeration
            for (const exp of mainModule.enumerateExports()) {{
                if (exp.name === symbolName || exp.name.includes('{match_fragment}')) {{
                    funcPtr = exp.address;
                    break;
                }}
            }}
        ...
        if (funcPtr) {{
            var ghidraOffset = getGhidraOffset(funcPtr, mainModule);
            console.log('[*] Found ' + handlerType + ' handler: {handler.demangled_name[:50]}');
            console.log('    Runtime: ' + funcPtr);
            console.log('    Ghidra: 0x' + ghidraOffset.toString(16));

            if (isSwift) {{
                console.log('    [!] WARNING: Swift function - hooking may cause crashes');
                console.log('    [!] Recommend static analysis in Ghidra');
            }}

            // Only hook if not Swift (safer approach)
            if (!isSwift) {{
                try {{
                    Interceptor.attach(funcPtr, {{
                        onEnter: function(args) {{
                            console.log('\\n[+] URL Handler called: {handler.demangled_name[:40]}');
                            console.log('    Type: ' + handlerType);
                            ...
                            for (var i = 0; i < Math.min(bt.length, 5); i++) {{
                                ...
                                var ghidraOff = getGhidraOffset(addr, mainModule);
                                console.log('      ' + addr + ' (Ghidra: 0x' + ghidraOff.toString(16) + ') ' + (sym.name || '<unknown>'));
                            }}
                        }}
                    }});
            ...
            console.log('[-] Handler not found: {handler.symbol_name[:40]}...');
        }}
    }})();
"""
